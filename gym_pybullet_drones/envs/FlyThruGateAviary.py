...
class FlyThruGateAviary(BaseRLAviary):
    ...
    def _computeReward(self):
        ...
        # If all the gates are passed, the episode will end in the next step, return 0
        if all(self.passing_flag):
            return 0.0
        ...
        delta_dist = prev_dist - cur_dist
        if delta_dist > 0:
            on_way_reward = 1.0 * delta_dist
        else:
            on_way_reward = delta_dist
        on_way_reward -= 0.001 * np.linalg.norm(ang_vel)
        on_way_reward -= 0.01 * (rpy[0]**2 + rpy[1]**2)
        passing = False
        for i, key in enumerate(self.racing_setup.keys()):
            if self.passing_flag[i]:
                continue
    ...
    '''def _computeReward(self):
        ...
        # We find the following gates
        for i, key in enumerate(self.racing_setup.keys()):
            if self.passing_flag[i]:
                continue
            gate_center = np.array(self.racing_setup[key][0])
            cur_dist = np.linalg.norm(gate_center - pos)
            prev_dist = np.linalg.norm(gate_center - self.prev_pos)
            break
        self.prev_pos = pos

        #1. Approaching the gate
        approach_reward = prev_dist - cur_dist

        #2. Penalty for angular velocity
        spin_penalty = 0.001 * np.linalg.norm(ang_vel)

        #3. Height reward (quadratic, peak at 0.6)
target_height = 0.6
height_error = pos[2] - target_height
        height_reward = 0.1 - 0.5 * height_error**2 # max 0.1 at exact height, decreasing quadratically

        #4. Penalty for vertical velocity moving away from the center
        v_z = vel[2]
if (pos[2] < target_height and v_z < 0) or (pos[2] > target_height and v_z > 0):
# Movement from the center
            vertical_penalty = -0.2 * abs(v_z)
else:
vertical_penalty = 0.0 # movement towards the center is not penalized
        ...
        for i, key in enumerate(self.racing_setup.keys()):
            ...
            if (self.racing_setup[key][1][0] < pos[0] < self.racing_setup[key][2][0] and
                self.racing_setup[key][1][1] < pos[1] < self.racing_setup[key][1][1] + 2*self.offset and
                self.offset < pos[2] < self.offset + self.h):
                passing = True
                self.passing_flag[i] = True
                break
        ...
        if passing:
            ...
            reward = approach_reward + height_reward + vertical_penalty - spin_penalty
    ...
    def _computeTruncated(self):
        ...
        """
        state = self._getDroneStateVector(0)
        if (abs(state[0]) > self.w/2 + 0.5 + 2*self.offset
        or state[1] > 6 or state[1] < 0
        or state[2] > self.h + self.offset or state[2] < self.offset # Truncate when the drone is too far away
        ):
    ...
    def _computeTerminated(self):
        ...
        """
        # just require to pass 4th gate
        if  self.passing_flag[3] == True or self.collide == True:
            return True
        else:
            return False
