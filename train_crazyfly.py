    ...
    while time_step <= max_training_timesteps:
        ...
        for i in range((env.EPISODE_LEN_SEC)*env.CTRL_FREQ):
            ...
            if done:
                ...
                # Проверка, пройдены ли все 4 ворота
        if hasattr(env, 'passing_flag') and all(env.passing_flag):
            success_path = os.path.join(log_dir, str(run_num), f"SUCCESS_{i_episode}_{time_step}_ppo_drone.pth")
            ppo_agent.save(success_path)
            print(f"\n🎉 УСПЕХ! Модель, пролетевшая все ворота, сохранена: {success_path}")
            # По желанию можно добавить остановку обучения, но по условию задачи просто сохраняем и продолжаем
            # Если хотите остановить обучение при первом успехе, раскомментируйте следующую строку:
            # max_training_timesteps = time_step  # или break с флагом
